 ...
 */
export const RISK: Record<DiffRisk, { label: string; chip: string; dot: string; order: number }> = {
  high: {
    label: "Высокий риск",
...
export const KIND: Record<Exclude<DiffKind, "same">, { label: string; chip: string }> = {
  modified: { label: "изменён", chip: "bg-blue-50 text-blue-700 border-blue-200" },
  added: { label: "добавлен", chip: "bg-green-50 text-green-700 border-green-200" },
  removed: { label: "удалён", chip: "bg-red-50 text-red-700 border-red-200" },
  moved: { label: "перенесён", chip: "bg-slate-100 text-slate-600 border-slate-200" },
};

export const ADVICE: Record<DiffAdvice, { label: string; chip: string }> = {
  accept: { label: "Можно принять", chip: "bg-green-50 text-green-700 border-green-200" },
  discuss: { label: "Обсудить", chip: "bg-amber-50 text-amber-700 border-amber-200" },
