  ...
  return (
    <div className="rounded-lg border bg-background">
      <div className="flex items-start gap-2 flex-wrap px-4 py-2.5 border-b">
        <span className="text-sm font-semibold">Пункт {item.clauseNumber ?? "—"}</span>
        {kind && (
          <span className={cn("px-1.5 py-0.5 rounded border text-[10px] font-medium", kind.chip)}>
            {kind.label}
          </span>
        )}
        {risk && (
          <span className={cn("px-1.5 py-0.5 rounded border text-[10px] font-medium", risk.chip)}>
            {risk.label}
          </span>
        )}
        {item.side && (
          <span className="text-[11px] text-muted-foreground">{SIDE[item.side]}</span>
        )}
        <button
      ...
      <div className="grid gap-px bg-border sm:grid-cols-2 border-t">
        <div className="bg-background px-4 py-2.5">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Было</div>
        ...
        <div className="bg-background px-4 py-2.5">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Стало</div>
          <div className="text-xs leading-relaxed whitespace-pre-wrap">
            {item.after ? (
