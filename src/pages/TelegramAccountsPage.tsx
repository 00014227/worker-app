  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {loading
              ? 'Загрузка…'
              : `${rows.length} аккаунтов для рассылки подрядчикам`}
          </p>
        </div>
      ...
      <div className="space-y-2.5">
        {rows.map((a) => (
          <AccountRow key={a.id} account={a} onChange={load} />
        ))}
  ...
  return (
    <Card>
      <CardHeader className="border-b pb-3">
        <CardTitle className="text-sm">Новый аккаунт</CardTitle>
      </CardHeader>
      <CardContent className="pt-1 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          API ID / Hash получаются на my.telegram.org. После создания нажмите
          «Войти» для авторизации по коду.
        </p>
      ...
      <CardContent className="space-y-2">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">
              Сегодня: {account.dailySentCount}
            </span>
            <span
              className={cn(
            ...
            {(account.status === 'loggedout' || account.status === 'blocked') &&
              step === 'idle' && (
                <button
                  onClick={sendCode}
                  disabled={busy}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium border border-border hover:bg-muted/50 disabled:opacity-40 transition-colors"
                >
                  {busy ? (
        ...
        {account.status === 'cooldown' && account.floodWaitUntil && (
          <div className="text-xs text-amber-600">
            Пауза до {new Date(account.floodWaitUntil).toLocaleString('ru-RU')}
          </div>
        )}
        ...
        {step === 'code' && (
          <div className="flex items-center gap-2 pt-1">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 disabled:opacity-40 transition-colors"
            >
              {busy ? (
