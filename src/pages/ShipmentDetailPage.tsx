...
function Field({
  ...
  return (
    <div>
      ...
      <p className={cn('text-sm mt-0.5 font-medium', mono && 'font-mono')}>
        {value}
      </p>
    </div>
  );
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      {(o.departure || o.destination) && (
        <div className="flex items-center gap-3 rounded-xl border bg-gradient-to-r from-muted/40 to-muted/10 px-5 py-4">
          <div className="flex-1 text-center">
            ...
            {o.departureDateActual && (
              <p className="text-xs text-green-600 mt-1">
                факт: {o.departureDateActual}
              </p>
            )}
            {!o.departureDateActual && o.departureDatePlan && (
              <p className="text-xs text-muted-foreground mt-1">
                план: {o.departureDatePlan}
              </p>
            )}
          </div>
          ...
          <div className="flex-1 text-center">
            ...
            {o.arrivalDateActual && (
              <p className="text-xs text-green-600 mt-1">
                факт: {o.arrivalDateActual}
              </p>
            )}
            {!o.arrivalDateActual && o.arrivalDatePlan && (
              <p className="text-xs text-muted-foreground mt-1">
                план: {o.arrivalDatePlan}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <CardContent className="space-y-3">
            ...
            {o.clientOrderNumber && (
              <Field
                ...
                value={o.clientOrderNumber}
                mono
              />
            )}
            {o.department && (
              <Field label="Подразделение" value={o.department} />
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <User size={14} /> Ответственные
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <Field label="Ответственный" value={o.responsible?.name} />
            <Field
              label="Ключевой аккаунт-менеджер"
              value={o.keyAccountManager?.name}
            />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Package size={14} /> Груз и транспорт
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {o.declaredWeight > 0 && (
              <Field
                label="Заявленный вес, кг"
                value={o.declaredWeight.toLocaleString('ru-RU')}
              />
            )}
            {o.actualWeight > 0 && (
              <Field
                label="Фактический вес, кг"
                value={o.actualWeight.toLocaleString('ru-RU')}
              />
            )}
            {o.vehicleCount > 0 && (
              <Field label="Кол-во ТС" value={o.vehicleCount} />
            )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <MapPin size={14} /> Отслеживание
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <Field label="Текущее местоположение" value={o.currentLocation} />
            <Field
              label="Трек-номер / накладная"
              value={o.waybillNumber}
              mono
            />
            {o.tracingComment && (
              <Field label="Комментарий трейсинга" value={o.tracingComment} />
            )}
          </CardContent>
        </Card>
      </div>
