    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Прайсы хранятся «как есть»; по ним ИИ считает стоимость в калькуляторе
          клиента.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <button
          ...
          disabled={
            uploading || !file || !name.trim() || transportTypes.length === 0
          }
            ...
            {sources.map((s) => (
              <tr key={s.id} className="border-b border-slate-50">
                ...
                <td className="px-4 py-3 text-slate-600">
                  {OFFICES.find((o) => o.value === s.office)?.label ?? s.office}
                </td>
                <td className="px-4 py-3 text-slate-600">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.isActive ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}
                  >
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <button
                      ...
                      title={s.isActive ? 'Выключить' : 'Включить'}
