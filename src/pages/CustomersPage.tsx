  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((c) => {
      ...
      return (
        ...
        (c.inn ?? '').includes(q) ||
        (c.reportEmails ?? '').toLowerCase().includes(q)
      );
    });
  }, [rows, search, onlyConfigured]);

  const { visible: visibleCustomers, ref } = useInfiniteScroll(
    filtered,
    `${search}|${onlyConfigured}`,
  );

  const configured = rows.filter(
    (c) => c.reportEmails && c.reportsEnabled,
  ).length;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {loading
              ? 'Загрузка…'
              : `${rows.length} клиентов · рассылка настроена у ${configured}`}
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="rounded-lg border bg-muted/30 px-4 py-2.5 text-xs text-muted-foreground">
        Клиентам с указанным email ежедневно в 11:00 уходит Excel с их
        перевозками в пути. Без email рассылка не идёт.
      </div>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {c.activeOrders > 0 ? (
                        <span className="inline-flex items-center gap-1 text-xs">
                          <Package size={11} /> {c.activeOrders}
                        </span>
                      ) : (
                        <span className="text-xs text-muted-foreground">—</span>
                      )}
...
function ReportPanel({
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const res = await customersApi.updateReports(customer.id, {
        reportEmails: emails.trim(),
        reportsEnabled: enabled,
      });
      onSaved({
        id: customer.id,
        reportEmails: res.reportEmails,
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="client@company.com, buh@company.com"
            />
